...
import {
  ICar, ICarsWithStatus, namePage, statusEngine,
} from '../type/type';
...
function Garage() {
  ...
  const initCars = (cars: ICar[]) => {
    const statusArray: ICarsWithStatus[] = cars.map((car) => ({ car, status: 'stopped', time: null }));
  ...
  const changeStatusOneCar = async (currentCar: ICarsWithStatus, command: statusEngine) => {
    ...
    const newCarsWithStatus: ICarsWithStatus[] = carsWithStatus.map((item) => {
      if (item.car.id === currentCar.car.id) {
        return { car: item.car, status: command, time: timeTripCar };
      }
          ...
          ? (
            <div className="winner__title">
              <div>
                &quot;
                {winner.car.name}
                &quot; - is a Winner! His time is
                {winner?.time}
                {' '}
                seconds
              </div>
              <Car color={winner.car.color} />
            </div>
          )
          : (
            <div className="winner__title">
